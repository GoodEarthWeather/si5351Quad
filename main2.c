...
void si5351_write_block(uint8_t start_reg, uint8_t *data, uint8_t len) {
    ...
    // 1. Send the register address to start the transaction
    EUSCI_B_I2C_masterSendMultiByteStart(EUSCI_B0_BASE, start_reg);

    // 2. Transmit all bytes except the very last one
    for(i = 0; i < (len - 1); i++) {
        EUSCI_B_I2C_masterSendMultiByteNext(EUSCI_B0_BASE, data[i]);
    }

    // 3. Send the final byte coupled with the STOP condition
    EUSCI_B_I2C_masterSendMultiByteStop(EUSCI_B0_BASE);
    EUSCI_B_I2C_masterSendMultiByteNext(EUSCI_B0_BASE, data[len - 1]);

    // 4. Wait until the bus clears completely
    while (EUSCI_B_I2C_isBusBusy(EUSCI_B0_BASE));
}
...
void enable_ReceiveMode(uint32_t target_frequency_hz) {
    ...
    uint32_t pll_multiplier = (target_frequency_hz * ms_divider) / 27000000; // Assuming 27MHz reference Xtal
...
void enable_TransmitMode(uint32_t target_frequency_hz) {
    uint32_t ms_divider = 32;
    uint32_t pll_multiplier = (target_frequency_hz * ms_divider) / 27000000;
